...
Planned v0 stack from `README.md` and `PRD.md`:
...
- Storage/output: filesystem-only run folders and Markdown reports
- LLM providers: OpenAI API and DeepSeek API
- Search providers: Brave Search first, DuckDuckGo where useful

This repository currently contains product/design documentation only; there is no source tree, `pyproject.toml`, test suite, CI, or implemented CLI yet. Do not invent commands or architecture beyond the PRD.

## Reference

Important files:

- `README.md` — high-level product vision, planned workflow, agent roles, verdicts, scoring, output structure, and design principles.
- `PRD.md` — detailed v0 product requirements, technology stack, CLI commands, storage layout, testing requirements, safety boundaries, and acceptance criteria.
...
Planned local output structure is `runs/<run-id>/` with `run.yaml`, `final-report.md`, `sources/`, `evidence/`, `opportunities/<slug>/`, and `appendix/` files as described in `PRD.md` section 15.

## Essential Commands

No project tooling is implemented yet. Use these status notes until scaffolding exists, and update this section as soon as real commands are added.

| Purpose | Current command |
| --- | --- |
| Build | TODO: add after Python package scaffolding exists. Do not claim a build command exists yet. |
...
| Test | TODO: add after tests exist, likely via `uv run pytest` if pytest is chosen. |
...
git status --short
git diff --check
grep -RInE "TODO|FIXME|HACK|don't|never|always" --exclude-dir=.git . || true
```

## Patterns and Workflows

- Follow the PRD before adding code. If implementation decisions conflict with `PRD.md`, stop and ask before changing product scope.
- Keep v0 local-first: write auditable files under `runs/<run-id>/`; do not add a database, scheduler, auth, web dashboard, or Docker unless the PRD changes.
...
No formatter or linter configuration exists yet. When Python scaffolding is added:
...
Commit history currently uses concise imperative summaries, e.g. `Initial commit` and `Add v0 PRD and expand project README`. Continue that style: `Add CLI scaffold`, `Implement source registry loading`, `Document retry state`.
...
4. For documentation-only changes, verify links, headings, command examples, and PRD consistency.
...
- Any scope changes from `PRD.md` or follow-up TODOs.
